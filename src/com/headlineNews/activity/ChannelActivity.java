...
import com.headlineNews.adapter.DragAdapter;
import com.headlineNews.adapter.OtherAdapter;
...
import com.headlineNews.widget.DragGrid;
import com.headlineNews.widget.OtherGridView;
...
public class ChannelActivity extends BaseActivity implements
	...
	private void initData() {
		...
		otherChannelList = ((ArrayList<Channel>) ChannelManage.getManage(
	...
	private void initView() {
		icon_collapse = (ImageView) findViewById(R.id.icon_collapse);
	...
	public void onItemClick(AdapterView<?> parent, final View view,
		...
		case R.id.userGridView:
			...
			if (position != 0 && position != 1) {
				...
				if (moveImageView != null) {
					...
					final Channel channel = ((DragAdapter) parent.getAdapter())
							.getItem(position);// 获取点击的频道内容
					otherAdapter.setVisible(false);
					// 添加到最后一个
					otherAdapter.addChannel(channel);
					new Handler().postDelayed(new Runnable() {
						public void run() {
							try {
								...
								userAdapter.setRemoveChannel(position);
		...
		case R.id.otherGridView:
			...
			if (moveImageView != null) {
				...
				// 添加到最后一个
				userAdapter.addChannel(channel);
				new Handler().postDelayed(new Runnable() {
					public void run() {
						try {
							int[] endLocation = new int[2];
							...
							otherAdapter.setRemoveChannel(position);
						} catch (Exception localException) {
						}
					}
				}, 50L);
		...
		moveAnimationSet.setAnimationListener(new AnimationListener() {
			...
			public void onAnimationEnd(Animation animation) {
				...
				if (clickGridView instanceof DragGrid) {
					...
					userAdapter.removeChannel();
				} else {
					userAdapter.setVisible(true);
					userAdapter.notifyDataSetChanged();
					otherAdapter.removeChannel();
				}
				isMove = false;
			}
		});
	...
	private void saveChannel() {
		ChannelManage.getManage(AppApplication.getApp().getSQLHelper())
				.deleteAllChannel();
		ChannelManage.getManage(AppApplication.getApp().getSQLHelper())
				.saveUserChannel(userAdapter.getChannelList());
		ChannelManage.getManage(AppApplication.getApp().getSQLHelper())
				.saveOtherChannel(otherAdapter.getChannelList());
	}
	...
	/** 关闭频道管理 */
	public void closeExpand(View view) {
		RotateAnimation animation = (RotateAnimation) AnimationUtils
				.loadAnimation(this, R.anim.close_manage_channel);
		icon_collapse.setAnimation(animation);
		onBackPressed();
	}
	/** 排序编辑 */
